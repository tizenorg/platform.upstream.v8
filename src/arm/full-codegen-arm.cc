...
void FullCodeGenerator::VisitForInStatement(ForInStatement* stmt) {
  ...
  bool accessOwnPropertyOnly = false;
  IfStatement* for_in_ifstmt = NULL;
  CompilationInfo *compInfo = compilationInfo();
  FunctionLiteral *lit = compInfo ? compInfo->function() : NULL;
  bool singleLoop = false;
  if (lit && compInfo->num_parameters() == 2
          && lit->GetNumForInLoops() == 1
          && lit->is_function()) { singleLoop = true; }
  do {
    if (!singleLoop) break;
    if (!IterativeOperations()) break;
    if (!stmt->each()->IsVariableProxy() || !stmt->subject()->IsVariableProxy())
      break;
    if (!stmt->body()->IsBlock()) break;
    Block* block = stmt->body()->AsBlock();
    if (block->statements()->length() != 1
        || !block->statements()->at(0)->IsIfStatement()) break;

    Handle<String> stmt_each_name = stmt->each()->AsVariableProxy()->name();
    Handle<String> stmt_subj_name = stmt->subject()->AsVariableProxy()->name();
    for_in_ifstmt = block->statements()->at(0)->AsIfStatement();
    if (!for_in_ifstmt->HasThenStatement() || for_in_ifstmt->HasElseStatement())
      break;
    if (!for_in_ifstmt->condition()->IsCall()) break;
    Call* call = for_in_ifstmt->condition()->AsCall();
    if (!call->expression()->IsProperty()) break;
    ZoneList<Expression*>* args = call->arguments();
    if (args->length() != 2) break;

    if (!args->at(0)->IsVariableProxy()) break;
    if (!String::Equals(stmt_subj_name, args->at(0)->AsVariableProxy()->name()))
      break;

    if (!args->at(1)->IsArrayLiteral()) break;
    ArrayLiteral* array = args->at(1)->AsArrayLiteral();
    if (array->values()->length() != 1) break;
    if (!array->values()->at(0)->IsVariableProxy()) break;
    if (!String::Equals(stmt_each_name,
                        array->values()->at(0)->AsVariableProxy()->name()))
      break;

    // Object.prototype.hasOwnProperty . apply
    Property* prop = call->expression()->AsProperty();
    if (!prop->obj()->IsProperty() || !prop->key()->IsLiteral()) break;
    Literal* lit = prop->key()->AsLiteral();
    if (!lit->value()->IsString()) break;
    Handle<String> key_string = Handle<String>::cast(lit->value());
    if (!String::Equals(key_string, isolate()->factory()->apply_string()))
      break;

    // Object.prototype . hasOwnProperty
    prop = prop->obj()->AsProperty();
    if (!prop->obj()->IsProperty() || !prop->key()->IsLiteral()) break;
    lit = prop->key()->AsLiteral();
    if (!lit->value()->IsString()) break;
    key_string = Handle<String>::cast(lit->value());
    if (!String::Equals(key_string,
                        isolate()->factory()->hasOwnProperty_string())) break;
    ...
    if (!lit->value()->IsString()) break;
    key_string = Handle<String>::cast(lit->value());
    if (!String::Equals(key_string, isolate()->factory()->prototype_string()))
      break;

    // Object
    VariableProxy* prox = prop->obj()->AsVariableProxy();
    if (!String::Equals(prox->name(), isolate()->factory()->Object_string()))
      break;

    accessOwnPropertyOnly = true;
  } while (0);
#endif
  ...
  if (accessOwnPropertyOnly) {
    __ CheckEnumCacheOwnProperty(null_value, &call_runtime);
  } else {
...
#ifdef SRUK_FOR_IN_LOOP
  if (accessOwnPropertyOnly) {
    __ CallRuntime(Runtime::kGetOwnPropertyNamesFast, 1);
  } else {
...
#ifdef SRUK_FOR_IN_LOOP
  if (accessOwnPropertyOnly) {
    __ JumpIfSmi(r3, &update_each);  // index r3 is a smi number ?
  }
#endif
...
#ifdef SRUK_FOR_IN_LOOP
  if (accessOwnPropertyOnly) {
    Visit(for_in_ifstmt->then_statement());
  } else {
    Visit(stmt->body());
  }
#else
